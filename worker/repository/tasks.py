...
@celery.task(name="tasks.retrieve_from_staging", bind=True)
def retrieve_from_staging(self, object_id, job_id):
    ...
    target = os.path.join(working_dir, job_id, object_id, "retrieve_from_staging")
    if os.path.exists(target):
        shutil.rmtree(target)
    shutil.copytree(source, target)


@celery.task(name="tasks.publish_to_repository")
