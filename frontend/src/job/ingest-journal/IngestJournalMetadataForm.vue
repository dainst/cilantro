    ...
    <section>
        <b-table :data="this.targets" detailed detail-key="id">
            <template slot-scope="props">
                ...
                <b-table-column field="id" label="Path">{{
                    props.row.id
                }}</b-table-column>
                <template
                    v-if="!isTargetError(props.row) && props.row.metadata"
                >
                    <b-table-column
                        field="metadata.description"
                        label="Description"
                        >{{
                            props.row.metadata.description || "-" | truncate(80)
                        }}</b-table-column
                    >
                    <b-table-column field="metadata.volume" label="Volume"
                    ...
                    <b-table-column field="metadata.number" label="Number"
                        >{{ props.row.metadata.number || "-" }}
                    </b-table-column>
                    <b-table-column field="metadata.zenon_link" label="Zenon"
                        ><a
                            :href="
                                'https://zenon.dainst.org/Record/' +
                                    props.row.metadata.zenon_id
                            "
                        >
            ...
            <template slot="detail" slot-scope="props">
                <div class="content metadata_output">
                    <ul>
                        <li v-for="(id, name) in props.row.metadata" :key="id">
                            {{ name }}: {{ id }}
                        </li>
                    </ul>
                    <ul v-if="isTargetError(props.row)">
                        <li
                            v-for="message in props.row.messages"
                            :key="message"
                        >
                            {{ message }}
                        </li>
...
import { JobTargetError, isTargetError } from '@/job/JobParameters';
import {
    MaybeJobTarget,
    JobTargetData,
...
export default class JournalMetadataForm extends Vue {
    ...
    async mounted() {
        this.targets = await asyncMap(
            this.selectedPaths,
            async(path): Promise<MaybeJobTarget> => {
                ...
                if (errors.length === 0) {
                    return new JobTargetData(id, path, {
                        zenon_id: zenonId
    ...
    removeTarget(removedTarget: MaybeJobTarget) {
        this.targets = this.targets.filter(
            target => removedTarget.id !== target.id
        );
        this.$emit('update:targetsUpdated', this.targets);
    }
}
...
function evaluateTargetFolder(targetFolder: WorkbenchFileTree) {
    ...
    if (containsNumberOfFiles(targetFolder, 0)) {
        errors.push(`Folder appears to be empty. Please provide input data.`);
    }

    if ('tif' in targetFolder) {
        // if there is a tif folder, make sure it only contains tifs
        if (
            targetFolder.tif.contents !== undefined &&
            !containsOnlyFilesWithExtensions(targetFolder.tif.contents, [
                '.tif',
                '.tiff'
            ])
        ) {
