...
def add_job(job_id, user, job_type, task_ids, job_params):
    ...
    job = {'job_id': job_id,
           'user': user,
           'job_type': job_type,
           'name': _generate_job_name(job_type, job_params['files']),
           'task_ids': task_ids,
           'state': 'new',
           'created': timestamp,
           'updated': timestamp,
...
def _generate_job_name(job_type, job_files):
    """
    Generate the job name based on type and files

    The job name is generated by appending the first and the last file names
    to the job_type, split by '-'.

    :param str job_type: type of the job
    :param list job_files: files of the job
    ...
    if len(file_list) > 1:
        name = f"JOB-{job_type}-{file_list[0]}-{file_list[-1]}"
    else:
        name = f"JOB-{job_type}-{file_list[0]}"
