...
import { MaybeJobTarget, JobTargetData, BookIssueMetadata } from './IngestBookParameters';
            ...
            this.selectedPaths, async(path) : Promise<MaybeJobTarget> => {
                ...
                if (errors.length === 0) {
                    return new JobTargetData(
                        id, path, { zenon_id: zenonId } as BookIssueMetadata
                    );
                }
...
async function loadZenonData(target: JobTargetData) {
    try {
        ...
        if (!zenonRecord.parentId) {
            errors.push(`Zenon record has no parent id. Can't determine which Book this issue belongs to.`);
        } else if (!(zenonRecord.parentId in ojsZenonMapping)) {
            errors.push(`Missing OJS Book code for Book with Zenon-ID '${zenonRecord.parentId}'.`);
        } else {
        ...
        const metadata = {
            ...
            ojs_Book_code: ojsZenonMapping[parentId],
            reporting_year: getReportingYear(zenonRecord)
        } as BookIssueMetadata;
